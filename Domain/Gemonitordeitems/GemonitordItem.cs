...
namespace Domain.Gemonitordeitems
{
    public class GemonitordItem
    {
        ...
        {
            int huidigaantalVermeldingen = ItemHistorieken.OrderByDescending(a => a.HistoriekDatum).Last().AantalVermeldingen;
            double gemiddeldAantalVermeldingen = ItemHistorieken.Average(a => a.AantalVermeldingen);
            if (gemiddeldAantalVermeldingen > huidigaantalVermeldingen * 0.9 && TotaalAantalVermeldingen < huidigaantalVermeldingen * 1.10)
            {
                VermeldingenTrend = Trend.NEUTRAL;
            }
